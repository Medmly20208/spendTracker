...
const HeaderNav = () => {
  ...
  const showNav = ()=>{setIsNavShown(true)}
  const hideNav = ()=>{setIsNavShown(false)}

  return (
    <div className='sm:hidden'>
        <Bars3BottomRightIcon className='w-[30px] h-[30px] text-gray-700 cursor-pointer' onClick={showNav}/>
        {isNavShown && 
        <>
        
         <div onClick={hideNav} className='fixed top-[0px] right-[0px] w-screen h-screen bg-black opacity-20 cursor-pointer '></div>
        <div className='fixed top-[0px] right-[0px] w-[50vw] h-screen bg-white  flex flex-col justify-center items-center gap-[20px] '>
         
        <nav className='w-[80%]'>
                <ul className='flex gap-[20px] flex-col text-center justify-stretch'>
                    <li className='w-full'><div className='flex items-start w-full'>
