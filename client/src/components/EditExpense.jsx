...
import { useEditExpenseMutation } from "../api/apiSlice";
...
const EditExpense = ({ onClose, id }) => {
  ...
  const [editExpense, { data, isSuccess, isError, error, isLoading }] =
  ...
  return (
    <div className="p-[20px] bg-white rounded-sm dark:bg-main-black ">
      ...
      <a
        className="mainBtn"
        onClick={isLoading ? () => {} : editExpenseHandler}
      >
        {isLoading ? "loading" : "Edit expense"}
      </a>
    </div>
  );
};
